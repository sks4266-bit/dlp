...
function getDescription(variant: HeaderNavVariant) {
  if (variant === 'terms') return '약관을 읽다가 개인정보 처리방침이나 문의 / 요청 접수 화면으로 자연스럽게 이동할 수 있도록 구성했습니다.';
  if (variant === 'privacy') return '권리 행사, 탈퇴, 개인정보 삭제 요청 전 필요한 정책과 접수 화면을 빠르게 연결합니다.';
  if (variant === 'support') return '접수 전에 이용약관과 개인정보 처리방침을 다시 확인하고, 필요하면 이메일이나 내정보 화면으로 이어질 수 있습니다.';
